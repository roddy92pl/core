	...
	public static function startStream($rStreamID, $rFromCache = false, $rForceSource = null, $rLLOD = false, $rStartPos = 0) {
		...
		if (self::$db->num_rows() > 0) {
			...
			if (self::$db->num_rows() > 0) {
				...
				// DASH/MPD często potrzebuje większego analizowania – pomoże przy streamach które nie startują
$firstSource = null;
if (!empty($rForceSource)) {
    $firstSource = $rForceSource;
} else {
    $tmpSources = json_decode($rStream['stream_info']['stream_source'], true);
    $firstSource = is_array($tmpSources) && !empty($tmpSources) ? $tmpSources[0] : null;
}
if ($firstSource && stripos($firstSource, '.mpd') !== false) {
	...
	// --- CENC per-source (format: http://..../file.mpd|cenc=HEX) ---
	// UWAGA: parsujemy |cenc= z ORYGINALNEGO $rSource, bo parseStreamURL() może ucinać nietypowe sufiksy
	$cencKey = null;
$cencKeys = [];
$sourceUrl = $rSource;

if (stripos($rSource, '|cenc=') !== false) {
    list($sourceUrl, $cencRaw) = explode('|cenc=', $rSource, 2);
    $sourceUrl = trim($sourceUrl);
    $cencRaw   = trim($cencRaw);

    // Obsługa wielu par kid:key rozdzielonych przecinkiem
    foreach (explode(',', $cencRaw) as $pair) {
        $pair = trim($pair);
        if (!empty($pair)) {
            $cencKeys[] = $pair;
        }
    ...
    // Kompatybilność wsteczna — pojedynczy klucz
    $cencKey = $cencKeys[0] ?? null;
}
...
if ($isDash && $hasCenc) {
    ...
    if (stripos($rFetchOptions, '-http_seekable 0') === false) {
      $rFetchOptions .= ' -http_seekable 0';
    }
...
if ($panelProxy !== '') {
    // FFmpeg
    if (stripos($rFetchOptions, '-http_proxy') === false) {
        $rFetchOptions .= ' -http_proxy ' . escapeshellarg($panelProxy);
    }
    // FFprobe (probe/scan source)
...
// Fallback: jeśli ffprobe nie dało codecs (często dla DASH/CENC),
// weź podstawowe info z "ffmpeg -i" (bez transkodowania).
if (
    (empty($rFFProbeOutput) || !isset($rFFProbeOutput['codecs']) || empty($rFFProbeOutput['codecs']['video'])) &&
    stripos($rStreamSource, '.mpd') !== false
) {
    $cmd = self::$rFFMPEG_CPU . ' -hide_banner -loglevel info -nostdin ' .
           $rFetchOptions . ' -i ' . escapeshellarg($rStreamSource) . ' -t 0 -f null - 2>&1';

    $out = shell_exec($cmd);

    // bardzo prosty parser: wykryj pierwszy video/audio codec
    $video = null; $audio = null; $height = null;

    if (preg_match('/Video:\s*([a-zA-Z0-9_]+)/', $out, $m)) {
        $video = strtolower($m[1]);
    }
    if (preg_match('/Audio:\s*([a-zA-Z0-9_]+)/', $out, $m)) {
        $audio = strtolower($m[1]);
    }
    if (preg_match('/,\s*(\d{3,4})x(\d{3,4})/', $out, $m)) {
        $height = intval($m[2]);
    }
    ...
    if ($video || $audio) {
        $rFFProbeOutput = array(
            'codecs' => array(
                'video' => array('codec_name' => $video ?: 'h264', 'codec_type' => 'video', 'height' => $height ?: 1080),
                'audio' => array('codec_name' => $audio ?: 'aac', 'codec_type' => 'audio')
            ),
            'container' => 'dash'
        );
    }
...
if (stripos($rStreamSource, '.mpd') !== false) {
    $rGenPTS = '-fflags +genpts -avoid_negative_ts make_zero';
					...
					// FIX: Radio (type=4) + HLS MPEGTS + audio copy can crash ("frame size not set").
// Force AAC for radio when audio codec is copy.
if (isset($rStream['stream_info']['type_key']) && $rStream['stream_info']['type_key'] === 'radio_streams') {
    if (isset($rStream['stream_info']['transcode_attributes']['-acodec']) &&
        $rStream['stream_info']['transcode_attributes']['-acodec'] === 'copy') {

        $rStream['stream_info']['transcode_attributes']['-acodec'] = 'aac';
        $rStream['stream_info']['transcode_attributes']['-b:a'] = '128k';
        $rStream['stream_info']['transcode_attributes']['-ar']  = '48000';
        $rStream['stream_info']['transcode_attributes']['-ac']  = '2';
    }
}
				...
				// HLS output: radio uses fMP4, other streams use MPEGTS
if (isset($rStream['stream_info']['type_key']) && $rStream['stream_info']['type_key'] === 'radio_streams') {
    $rOutputs['mpegts'][] =
        $rOptions .
        ' -individual_header_trailer 0 -f hls' .
        ' -hls_time ' . intval(self::$rSegmentSettings['seg_time']) .
        ' -hls_list_size ' . intval(self::$rSegmentSettings['seg_list_size']) .
        ' -hls_delete_threshold ' . intval(self::$rSegmentSettings['seg_delete_threshold']) .
        ' -hls_flags delete_segments+discont_start+omit_endlist' . $rKeyFrames .
        ' -hls_segment_type fmp4' .
        ' -hls_fmp4_init_filename "' . intval($rStreamID) . '_init.mp4"' .
        ' -hls_segment_filename "' . STREAMS_PATH . intval($rStreamID) . '_%d.m4s"' .
        ' "' . STREAMS_PATH . intval($rStreamID) . '_.m3u8" ';
} else {
    $rOutputs['mpegts'][] =
        $rOptions .
        ' -individual_header_trailer 0 -f hls -hls_time ' . intval(self::$rSegmentSettings['seg_time']) .
        ' -hls_list_size ' . intval(self::$rSegmentSettings['seg_list_size']) .
        ' -hls_delete_threshold ' . intval(self::$rSegmentSettings['seg_delete_threshold']) .
        ' -hls_flags delete_segments+discont_start+omit_endlist' . $rKeyFrames .
...
public static function normalizeHttpProxy($proxyRaw) {
    $proxyRaw = trim((string)$proxyRaw);
    if ($proxyRaw === '') return '';

    $proxyRaw = preg_replace('/\s+/', '', $proxyRaw);

    // jeśli brak schematu, dopnij http://
    if (!preg_match('#^[a-zA-Z][a-zA-Z0-9+\-.]*://#', $proxyRaw)) {
        $proxyRaw = 'http://' . $proxyRaw;
    }

    // pozwól tylko http/https
    if (stripos($proxyRaw, 'http://') !== 0 && stripos($proxyRaw, 'https://') !== 0) {
        return '';
    }

    $parts = @parse_url($proxyRaw);
    if (!$parts || empty($parts['host']) || empty($parts['port'])) {
        return '';
    }
...
if ($rArgument['argument_key'] == 'headers') {
    $v = trim((string)$rArgument['value']);

    // normalizacja końców linii
    $v = str_replace(["\r\n", "\r"], "\n", $v);

    // jeśli ktoś podał w jednej linii (CSV) → rozbij po przecinku
    if (strpos($v, "\n") === false) {
        $headers = array_map('trim', explode(',', $v));
    } else {
        // jeśli już są linie → użyj ich
        $headers = array_map('trim', explode("\n", $v));
    }

    // usuń puste / śmieci
    $headers = array_filter($headers, function($h) {
        return $h !== '' && strpos($h, ':') !== false;
    });

    // sklej do formatu FFmpeg (CRLF!)
    $v = implode("\r\n", $headers) . "\r\n";
